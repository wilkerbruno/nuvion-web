...
| `CORS_ALLOWED_ORIGINS` | não (default só localhost) | a(s) URL(s) do painel publicado — **sem isso o painel em produção não consegue chamar a API** (CORS bloqueado, erro `400` em `OPTIONS /auth/login` no navegador). Aceita tanto JSON (`["https://app.seudominio.com.br"]`) quanto uma URL simples (`https://app.seudominio.com.br`) ou uma lista separada por vírgula (`https://app.seudominio.com.br,http://localhost:3000`) — não precisa ser JSON perfeito, o backend tolera os formatos comuns de digitar isso numa caixa de variável de ambiente |
