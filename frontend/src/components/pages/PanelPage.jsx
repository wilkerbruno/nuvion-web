...
import {
  LayoutGrid, List, Search, Trash2, ExternalLink,
  Loader2, Plus, X, Monitor,
} from "lucide-react";
import clsx from "clsx";

const NOVNC_URL = import.meta.env.VITE_WORKER_NOVNC_URL || "https://divisions-nuvion-worker.lcgx8u.easypanel.host";
...
// ── ToolViewer (noVNC overlay) ────────────────────────────────────────────────
function ToolViewer({ toolName, onClose }) {
  const novncSrc = `${NOVNC_URL}/vnc.html?autoconnect=true&reconnect=true&resize=scale&quality=6&compression=2&view_only=false`;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#fff", fontSize: 14 }}>
          ...
          <span style={{ fontWeight: 600 }}>{toolName || "Chrome Remoto"}</span>
          <span style={{
...
function ToolCard({ tool, onOpen, onFavorite, onDelete, isAdmin }) {
  ...
  const handleOpen = async () => {
    setOpening(true);
    try { await onOpen(tool.id, tool.name); }
  ...
  return (
    ...
    >
      <div className="absolute top-3 right-3 flex gap-1.5" onClick={(e) => e.stopPropagation()}>
        <button onClick={() => onFavorite(tool.id)} className="text-lg leading-none hover:scale-110 transition-transform">
          {tool.is_favorite ? "❤️" : "🤍"}
        </button>
        {isAdmin && (
          <button onClick={() => onDelete(tool.id)} className="text-slate-500 hover:text-red-400 transition-colors">
            <Trash2 size={15} />
          </button>
        )}
      </div>
      ...
      <div className="flex items-center gap-1.5 flex-wrap mt-1">
        <span className="text-[11px] font-semibold text-white px-2.5 py-0.5 rounded-full" style={{ backgroundColor: color }}>
          {tool.category}
        </span>
        {(tool.tags || []).slice(0, 1).map((tag) => (
          <span key={tag} className="text-[11px] bg-[#1E293B] text-slate-300 px-2 py-0.5 rounded-full">{tag}</span>
        ))}
      </div>
      ...
      <div className="flex items-center justify-end text-[11px] text-slate-500 group-hover:text-slate-300 transition-colors mt-auto pt-1">
        {opening ? <Loader2 size={13} className="animate-spin" /> : <><ExternalLink size={11} className="mr-1" /> Abrir no Chrome</>}
      </div>
    </div>
  );
}
...
function ToolListItem({ tool, onOpen, onFavorite, onDelete, isAdmin }) {
  const color = CATEGORY_COLORS[tool.category] || "#334155";
  return (
    <div className="flex items-center gap-4 px-4 py-3 rounded-xl border border-[#1E293B] bg-[#0F172A] hover:bg-[#1A3A5C] hover:border-[#2563EB] transition-all cursor-pointer"
      onClick={() => onOpen(tool.id, tool.name)}>
      ...
      <div className="w-40 shrink-0">
        <p className="font-semibold text-white text-sm truncate">{tool.name}</p>
        <p className="text-slate-400 text-xs truncate">{tool.url.replace("https://", "").replace("http://", "").slice(0, 30)}</p>
      </div>
      <span className="text-[11px] font-semibold text-white px-3 py-1 rounded-full shrink-0" style={{ backgroundColor: color }}>
        {tool.category}
      </span>
      <div className="flex-1" />
      <div className="flex items-center gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
        <button onClick={() => onFavorite(tool.id)} className="text-base">{tool.is_favorite ? "❤️" : "🤍"}</button>
        {isAdmin && <button onClick={() => onDelete(tool.id)} className="text-slate-500 hover:text-red-400"><Trash2 size={15} /></button>}
      </div>
    </div>
  );
}
...
export default function PanelPage() {
  ...
  // Estado do viewer noVNC
  const [viewer, setViewer] = useState(null); // { toolName }

  useEffect(() => { fetchTools(); }, [category]);

  const handleOpen = useCallback(async (toolId, toolName) => {
    ...
    try {
      ...
      ws.onerror = () => toast.error("Erro de conexão com o worker", { id: toastId });
  ...
  return (
    <>
      {/* noVNC overlay — aparece quando viewer != null */}
      {viewer && <ToolViewer toolName={viewer.toolName} onClose={() => setViewer(null)} />}
        ...
        <aside className="w-56 shrink-0 bg-[#0B1120] border-r border-[#1E293B] py-4 px-2 overflow-y-auto">
          {CATEGORIES.map((cat) => (
            <button key={cat.id} onClick={() => setCategory(cat.id)}
              className={clsx("w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium transition-colors mb-0.5",
                category === cat.id ? "bg-[#2563EB] text-white" : "text-slate-400 hover:bg-[#1E293B] hover:text-white")}>
              {cat.label}
            </button>
          ))}
        </aside>
        ...
        <div className="flex-1 flex flex-col overflow-hidden p-6 gap-4">
          ...
          {loading ? (
              ...
              <div className="flex flex-wrap gap-5">
                {tools.map((tool) => (
                  <ToolCard key={tool.id} tool={tool} onOpen={handleOpen}
                    onFavorite={handleFavorite} onDelete={handleDelete} isAdmin={isAdmin} />
                ))}
              </div>
            </div>
          ) : (
            <div className="flex-1 overflow-auto flex flex-col gap-2">
              {tools.map((tool) => (
                <ToolListItem key={tool.id} tool={tool} onOpen={handleOpen}
                  onFavorite={handleFavorite} onDelete={handleDelete} isAdmin={isAdmin} />
              ))}
            </div>
          )}
